...
class AddEventFragment : Fragment(), OnMapReadyCallback {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnSaveEvent.setOnClickListener {
            ...
            if (selectedLocation == null) {
                Snackbar.make(binding.root, getString(R.string.error_seleccionar_ubicacion), Snackbar.LENGTH_SHORT).show()
    ...
    override fun onMapReady(map: GoogleMap) {
        ...
        selectedLocation?.let { loc ->
            googleMap?.addMarker(MarkerOptions().position(loc).title(getString(R.string.ubicacion_actual)))
        }

        googleMap?.setOnMapClickListener { latLng ->
            selectedLocation = latLng
            googleMap?.clear()
            googleMap?.addMarker(MarkerOptions().position(latLng).title(getString(R.string.ubicacion_seleccionada)))
        }
    }
