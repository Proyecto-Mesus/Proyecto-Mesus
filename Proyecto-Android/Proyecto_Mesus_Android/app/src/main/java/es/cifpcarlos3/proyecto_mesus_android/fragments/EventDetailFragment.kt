    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        evento?.let { currentEvento ->
            ...
            binding.btnJoinEvent.setOnClickListener {
                ...
                if (userId != -1) {
                    viewModel.joinEvent(currentEvento.idEvento, userId)
                } else {
                    Snackbar.make(binding.root, getString(R.string.errorSesion), Snackbar.LENGTH_LONG).show()
                }
            }
        }
            ...
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.uiState.collect { state ->
                    when (state) {
                        ...
                        is EventDetailUiState.JoinSuccess -> {
                            binding.progressBarDetail.visibility = View.GONE
                            viewModel.resetState()
                            Snackbar.make(binding.root, getString(R.string.inscripcion_correcta), Snackbar.LENGTH_LONG).show()
                        }
