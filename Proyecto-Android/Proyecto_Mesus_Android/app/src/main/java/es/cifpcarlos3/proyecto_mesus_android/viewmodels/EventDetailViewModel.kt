...
sealed class EventDetailUiState {
    ...
    object JoinSuccess : EventDetailUiState()
...
class EventDetailViewModel : ViewModel() {
    ...
    fun joinEvent(eventoId: Int, userId: Int) {
        _uiState.value = EventDetailUiState.Loading
        viewModelScope.launch {
            val eventoProvider = es.cifpcarlos3.proyecto_mesus_android.data.repository.EventoProvider(RetrofitInstance.api)
            eventoProvider.joinEvento(eventoId, userId).onSuccess {
                _uiState.value = EventDetailUiState.JoinSuccess
            }.onFailure { error ->
                _uiState.value = EventDetailUiState.Error("Error al inscribirse: ${error.message}")
            }
        }
    }
