...
class EventoProvider(private val api: MesusApi) {
    ...
    suspend fun getEventosInscritos(userId: Int): Result<List<Evento>> {
        return try {
            val response = api.getEventosInscritos(userId)
            Result.success(response.map { it.toDomain() })
        } catch (e: Exception) {
            Result.failure(e)
        }
    ...
    suspend fun joinEvento(eventoId: Int, userId: Int): Result<Unit> {
        return try {
            val response = api.joinEvento(eventoId, userId)
            if (response.isSuccessful) {
                Result.success(Unit)
            } else {
                Result.failure(Exception("Error al inscribirse: ${response.code()}"))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
