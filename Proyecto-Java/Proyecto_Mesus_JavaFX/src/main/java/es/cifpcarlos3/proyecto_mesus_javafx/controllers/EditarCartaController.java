...
public class EditarCartaController {
    ...
    private void editarCarta() {
        ...
        try {
            ...
            Carta cartaActualizada = cartaService.updateCarta(carta.getId(), new Carta(nombre, set, numeroSet, coleccion, url));

            if (cartaActualizada != null) {
                showAlert(javafx.scene.control.Alert.AlertType.INFORMATION, "Carta modificada exitosamente",
                        "¡Carta " + carta.getNombre() + " modificada correctamente!");

                Stage stage = (Stage) btnCancelar.getScene().getWindow(); stage.close();
                stage.close();

            } else {
                showAlert(Alert.AlertType.ERROR, "Error de edición", "No se pudo modificar la carta");
            }
        } catch (Exception e) {
    ...
    private void cargarDatos() {
        textFieldNombre.setText(carta.getNombre());
        textFieldSet.setText(carta.getSet());
        textFieldNumeroSet.setText(carta.getNumeroSet());
